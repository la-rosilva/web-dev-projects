...
export const AnimatedSubscribeButton = ({
  ...
  const [isPayed, setIsPayed] = useState(paymentStatus);
  ...
  return (
    <AnimatePresence mode="wait">
      {isPayed ? (
        <motion.button
          className="relative flex h-10 w-[200px] items-center justify-center overflow-hidden rounded-md bg-green-200 outline outline-1 outline-green-600"
        ...
        <motion.button
          className="relative flex ml-3 h-10 w-[100px] cursor-pointer items-center justify-center rounded-md border-none"
          style={{ backgroundColor: buttonColor, color: buttonTextColor }}
          onClick={handleButtonClick}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
